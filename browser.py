...
# Расширенные темы с новыми цветами для индикаторов
THEMES = {
...
class EnhancedGifLoadingWidget(QWidget):
    ...
    def _load_gif_or_fallback(self):
        ...
        # Если GIF не найден, создаем анимированный QMovie из фреймов
    ...
    def _create_animated_fallback(self):
        ...
        # Создаем 12 кадров вращающегося круга
        for i in range(12):
        ...
        self._static_frame = self._frames[0]
        self._show_static()
    ...
    def start(self):
        ...
        if self.movie:
            self.movie.start()
        elif hasattr(self, '_animation_timer'):
...
# Безопасное шифрование паролей с Fernet
...
class NostalgiaPage(QWebEnginePage):
    ...
    def __init__(self, site_filter, scheme_handler,
        ...
        # Подключаем обработку ошибок сертификатов
...
class ColoredTabLoadingWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Выбор цвета в зависимости от стадии загрузки
        if self.loading_stage == 0:  # Подключение
            color = QColor(255, 165, 0)  # Оранжевый
        elif self.loading_stage == 1:  # Загрузка данных
            color = QColor(0, 80, 200)  # Синий
        else:  # Отрисовка
...
class NostalgiaBrowser(QMainWindow):
    ...
    def update_progress(self, progress: int, tab: BrowserTab):
        ...
        # Обновляем стадию загрузки для цветового индикатора
        if tab in self.loading_tabs:
            if progress < 30:
                self.loading_tabs[tab].set_stage(0)  # Подключение
            elif progress < 80:
                self.loading_tabs[tab].set_stage(1)  # Загрузка
            else:
                self.loading_tabs[tab].set_stage(2)  # Отрисовка
    ...
    def on_url_changed(self, url: QUrl, tab: BrowserTab):
        if self.tab_widget.currentWidget() is tab:
            ...
            # Визуальная индикация безопасности URL
            if url.scheme() == "https":
    ...
    def switch_to_tab(self, index: int):
        """Переключение на вкладку по индексу (0-7)."""
